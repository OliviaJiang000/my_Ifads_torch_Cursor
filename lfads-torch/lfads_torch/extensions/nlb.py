...
class NLBEvaluation(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if behavior.ndim < 3:
            ...
            # 检查是否存在 decode_mask，如果不存在则跳过 behavior_r2 计算
            if hasattr(trainer.datamodule, "train_decode_mask") and hasattr(trainer.datamodule, "valid_decode_mask"):
                ...
                # 如果没有 decode_mask，计算 tp_corr 作为替代
                tp_corr = speed_tp_correlation(heldout, rates_obs, behavior)
                metrics["nlb/tp_corr"] = tp_corr
